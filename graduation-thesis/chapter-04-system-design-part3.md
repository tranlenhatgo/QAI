...
## 4.16 Adaptive Infinity Quiz Design

Adaptive Infinity Quiz is a signed-in, single-category practice loop that combines static local questions, AI-generated continuation batches, and wrong-question repeats.

### 4.16.1 Session Initialization

```text
1. User enables Infinity Quiz and selects one category.
2. Frontend loads static questions from questions.json.
3. Frontend asks Spring Boot for /adaptive-practice/session-state.
4. If no static history exists, enqueue unanswered static questions first.
5. If static history exists, request AI questions immediately.
```

The static ID format is `<category-id>:<hash(question + correctAnswer)>`, which lets the backend report which local questions have already been answered without storing the entire static bank.

### 4.16.2 AI Batch Contract

```text
Frontend -> BFF -> AI Coach /generate/adaptive-questions
  category: selected category only
  Lite: count 5, history/wrong/recent context limited to 5 items
  Full: count 10, history/wrong/recent context limited to 20 items
```

Lite users route to LM Studio with `qwen/qwen3.5-9b`. Full users route to the Full provider. If the selected provider is unavailable, the route returns an error instead of silently falling back to another tier.

### 4.16.3 In-Session Spaced Repetition

Wrong questions are reinserted as exact question objects:

- First wrong answer: repeat after two intervening questions.
- Wrong again on repeat: repeat after four intervening questions.
- Correct on repeat: remove from the active wrong queue.

Adaptive answers are saved to `users/{uid}/adaptive_practice_answers` and intentionally do not create formal quiz attempt, review schedule, notification, or profile history records.

## 4.17 Notification System Design
