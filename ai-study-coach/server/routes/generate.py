...
from server.adaptive.schemas import AdaptiveQuestionsRequest, AdaptiveQuestionsResponse
...
async def generate_adaptive_questions(req: AdaptiveQuestionsRequest):
    ...
    provider = create_llm_provider(resolved_tier)
    result = await generate_adaptive_pipeline(req, resolved_tier, provider)
    return AdaptiveQuestionsResponse(questions=result.questions)
