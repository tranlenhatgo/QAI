...
- **Tier entitlement**: Lite/Full subscription enforcement happens before requests reach the coach. Next.js and Spring Boot coerce unauthorized Full requests to Lite or block Full-only RAG ingestion; coach routes still treat incoming `tier` as the runtime model selector.
