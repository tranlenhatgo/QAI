...
```python
# ai_coach/capabilities/agentic.py
...
- [ ] Works with Gemini's native function calling format

---

## Dependencies

- `ai_coach/llm/base.py` — LLMService, Message, StreamChunk, ToolCall
- `ai_coach/tools/base.py` — BaseTool
- `ai_coach/tools/registry.py` — ToolRegistry
...
Your AI Study Coach uses **native function calling** (Gemini's `function_call` response type). This is:

- More reliable (structured output, not text parsing)
- Simpler code (no label parsing, no regex)
- Only works with providers that support it (Gemini ✅, LM Studio ❌)
