...
Server resolves:
  ...
  tier=full  + mode=chat    → SimpleChatCapability(provider=gemini)
  tier=full  + mode=agentic → AgenticCapability(provider=gemini, tools=ALL)
```
...
def resolve_capability(tier: Tier, mode: Mode) -> BaseCapability:
    ...
    if mode == Mode.CHAT:
        provider = "lm_studio" if tier == Tier.LITE else "gemini"
        return SimpleChatCapability(provider=provider)
    
    if tier == Tier.LITE:
        return LiteOrchestrator(provider="lm_studio")
    else:
        return AgenticCapability(provider="gemini", tools=FULL_TOOL_SET)
```
...
- [ ] Gemini API key missing → Full mode returns graceful error
