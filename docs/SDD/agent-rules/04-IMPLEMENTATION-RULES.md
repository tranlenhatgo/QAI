...
ai_coach/capabilities/agentic.py  # 750 lines ❌

# After (split into package):
ai_coach/capabilities/agentic/
...
ai_coach/capabilities/lite_orchestrator.py
...
├── tests/                       ← Mirrors ai_coach/ structure
├── pyproject.toml
├── requirements.txt
├── .env.example
└── README.md
...
fix: handle Gemini rate limit with retry
...
logger.info(f"API key: {config.gemini_api_key}")    # ❌ NEVER
