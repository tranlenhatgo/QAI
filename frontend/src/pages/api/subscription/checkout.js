...
async function handler(req, res) {
	...
	try {
		const response = await fetch(`${apiRoot}/subscription/checkout`, {
			method: 'POST',
			headers: {
				Authorization: `Bearer ${req.idToken}`,
				'Content-Type': 'application/json',
			},
			body: JSON.stringify({ plan: req.body?.plan }),
		})
		...
		console.error('[subscription/checkout] Failed to proxy request', error.message)
