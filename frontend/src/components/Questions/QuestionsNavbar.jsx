...
export default function QuestionsNavbar ({ changueCurrent }) {
	const { queries, questions, currentQuestion, questionProgress } = useBoundStore(state => state)

	if (queries.infinitymode) {
		return <div className='mx-auto bg-white text-black rounded-full z-10 w-14 grid place-items-center aspect-square mb-4 text-xl font-medium'>{questionProgress}</div>
	}
